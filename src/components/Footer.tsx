    ...
    <footer data-section="footer" data-agapps-id="global-footer-container" className="bg-gray-900 text-gray-400 py-12">
      <Container>
        <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-8">
          <div>
            <span data-role="logo" data-agapps-id="global-footer-logo" data-agapps-key="brand" className="text-white font-bold text-lg block mb-2">{t("brand")}</span>
            <p data-agapps-id="global-footer-tagline" data-agapps-key="footer.tagline" className="text-sm max-w-xs">
              {t("footer.tagline")}
            </p>
          </div>
          <nav className="flex flex-col gap-2 text-sm">
            {navLinks.map((link) => (
              <a key={link.href} href={link.href} data-role="nav-link" data-agapps-id="global-footer-nav-link" data-agapps-key={link.labelKey} className="hover:text-white transition-colors">
                {t(link.labelKey)}
              </a>
            ))}
          </nav>
          <address className="not-italic text-sm space-y-1">
            ...
            <p data-role="address" data-agapps-id="global-footer-address" data-agapps-key="footer.address">{t("footer.address")}</p>
          </address>
        </div>
