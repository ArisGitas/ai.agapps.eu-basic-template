    ...
    <footer data-section="footer" className="bg-gray-900 text-gray-400 py-12">
      <Container>
        <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-8">
          <div>
            <span data-role="logo" className="text-white font-bold text-lg block mb-2">{t("brand")}</span>
          ...
          <nav className="flex flex-col gap-2 text-sm">
            {navLinks.map((link) => (
              <a key={link.href} href={link.href} data-role="nav-link" className="hover:text-white transition-colors">
                {t(link.labelKey)}
              </a>
            ))}
          </nav>
          <address className="not-italic text-sm space-y-1">
            ...
            <p data-role="address">{t("footer.address")}</p>
          </address>
        </div>
